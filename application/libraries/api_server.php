 ...
 * @author Himel
 */
require APPPATH.'core/Providers/GP_1.php';
...
class Api_server {
    ...
    public function save_message_data($xmlObj)
    {
        ...
        foreach($xmlObj as $node){
            ...
            $context = new Context();
            $context->setData((string)$node->subject,$message->mobileNumber);
            $lineNumber = trim($context->getLineNumber());

            /*if($lineNumber == "GP_1")
            {
                $provider = new $lineNumber;
                //$test = $provider->getMaskList();
                $message->subject = $context->getSubject($provider);
            }else{
                $message->subject = (string)$node->subject;
            }*/
    ...
    public function get_sms_balance($clientId)
    {
        $this->CI->load->model('smsreport');
        $smsReport = new SmsReport();
        return $smsReport->getSMSBalance($clientId);
    }
    
    public function get_daily_sms_count($clientId,$date)
    {
    	$this->CI->load->model('smsreport');
    	$smsReport = new SmsReport();
    	$result =  $smsReport->getReportToday($clientId,$date,false,1);
        return $result;
    }
